...
public class SamlConfiguration {
	...
	private List<RelyingPartyRegistration> getRelyingParties(List<SampleRelyingParty> sampleRelyingParties) {
		...
		return sampleRelyingParties.stream().map(p -> StringUtils.hasText(p.getLocalSpEntityIdTemplate())
				? RelyingPartyRegistration.withRegistrationId(p.getRegistrationId())
						.assertionConsumerServiceUrlTemplate(acsUrlTemplate).remoteIdpEntityId(p.getEntityId())
						.idpWebSsoUrl(p.getWebSsoUrl()).credentials(c -> c.addAll(p.getProviderCredentials()))
						.localEntityIdTemplate(p.getLocalSpEntityIdTemplate()).build()
				: RelyingPartyRegistration.withRegistrationId(p.getRegistrationId())
						.assertionConsumerServiceUrlTemplate(acsUrlTemplate).remoteIdpEntityId(p.getEntityId())
						.idpWebSsoUrl(p.getWebSsoUrl()).credentials(c -> c.addAll(p.getProviderCredentials())).build())
