...
@Slf4j
public class ExternalIdpAuthenticationFailureHandlerTest implements IExternalIdpAuthenticationFailureHandler {
	 ...
	 * ->504 Gateway Timeot
	 * 
	 * CancelAuthFilter codes(error) to message
	 */
	...
	private AuthenticationFailureHandler loginTimeout;
	...
	public ExternalIdpAuthenticationFailureHandlerTest(IdpConfigurationProperties idpConfiguration) {
		log.debug("Profile('DEV') IExternalIdpAuthenticationFailureHandler");
		this.idpConfiguration = idpConfiguration;
		
		this.badCredentials = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=403");
		this.missingTfaProfile = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=404.1");
		...
		this.missingMessage = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=408");
		this.loginTimeout = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=440");
		this.serverErrorAuth = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=500");
		this.gatewayTimeout = new SimpleUrlAuthenticationFailureHandler(this.idpConfiguration.getPathPrefix() + "/eauth/ssologin?error=504");
